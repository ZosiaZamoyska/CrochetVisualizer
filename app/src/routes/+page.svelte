<script>
  ...
  import { parsePattern, grid } from '../lib/parser.js';
  import { createP5Instance } from '../lib/p5Sketch.js';
  import '$lib/App.css';
  ...
  onMount(async () => {
    // Check if there's a pattern to load from the patterns page
    const patternToLoad = localStorage.getItem('patternToLoad');
    if (patternToLoad) {
      const pattern = JSON.parse(patternToLoad);
      ...
      localStorage.removeItem('patternToLoad'); // Clear the loaded pattern
    }
